...
{% set table_name = source_relation.database~'.'~source_relation.schema~'.RAW_'~source_relation.identifier %}
CREATE OR REPLACE TABLE {{table_name}} (
WAREHOUSE VARCHAR(3),
{%- for column in columns %}
    {% if  column.is_string() == True %}
...
SELECT 'ATL',*,current_timestamp as MODIFICATION_TIMESTAMP FROM {{ source(source_name, source_table_name) }};
